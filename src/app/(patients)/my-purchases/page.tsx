    ...
    <>
      <WrapperBanner>
        ...
        <div className="flex px-7.5 flex-wrap md:gap-0 gap-4 pt-5.5 pb-7.5">
              ...
              {!filteredData.length && !landing && <div className="col-span-5 text-center text-gray-500">No purchases found.</div>}
              { filteredData.map((item) => (
                ...
                >
                 ...
                 <FaClock/>{item.remaining_time}
                </span>
                  <div className="bg-[#B8E1D9] rounded-[10px] h-30 flex items-center justify-center mb-2.5">

                    {item.type === "Video" ? ( <FaVideo className="text-primary w-15 h-15" />) :
                  ...
                  <h3 className="text-primary text-sm font-semibold mb-2.5">
                    {item.title}
                         ...
                         <p className="text-gray-500 text-xs">Expires: {item.end_time}</p>
      ...
      <ResourceVideoPlayer  isOpen={selectedSessionVideo}  onClose={()=>setSelectedSessionVideo("")} />

      <ResourceReflectionQ  isOpen={selectedSessionRQ}  onClose={()=>setSelectedSessionRQ("")} />
    </>
  );
}
