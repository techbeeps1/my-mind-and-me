...
export default function Resources() {
  ...
  useEffect(() => {
    ...
    GetResources("Reflection Questions", MMMUserData?.id).then((data) => {
      if(data.success){
  ...
  return (
    <>
            ...
            <h2 className="text-center rounded-t-[10px] bg-[linear-gradient(90deg,#56e1e845_70%,var(--color-background)_100%)]  w-full text-primary md:text-[25px] text-[20px] leading-9 py-3 font-semibold">
              Reflection Questions 
                    ...
                    <div className="flex px-7.5 flex-wrap md:gap-0 gap-4 pt-5.5 pb-7.5">
                          ...
                          {!filteredData.length && !landing && <div className="col-span-5 text-center text-gray-500">No reflection questions found.</div>}
      ...
      <ResourceReflectionQ  isOpen={selectedSessionFree}  onClose={()=>setSelectedSessionFree("")} />
        
      <ResourcePaid 
        key={selectedSessionPaid} data={{
        id: selectedSessionPaid,
        name:   Resources.find((item) => item.id === selectedSessionPaid)?.title || "",
        type: "Reflection Questions",
        amount:  Resources.find((item) => item.id === selectedSessionPaid)?.price ||"",
      }} isOpen={selectedSessionPaid} onClose={()=>setSelectedSessionPaid("")} />
